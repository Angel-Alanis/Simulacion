...
// Datos de niveles
const levels = [
  { name: 'Beginner', order: 1, minQuestions: 1, maxFailures: 1, description: 'Nivel básico de inglés' },
  { name: 'Elementary', order: 2, minQuestions: 11, maxFailures: 2, description: 'Nivel elemental' },
  { name: 'Pre-intermediate', order: 3, minQuestions: 26, maxFailures: 2, description: 'Nivel pre-intermedio' },
  { name: 'Intermediate', order: 4, minQuestions: 41, maxFailures: 3, description: 'Nivel intermedio' },
  { name: 'Upper-intermediate', order: 5, minQuestions: 61, maxFailures: 1, description: 'Nivel intermedio-avanzado' },
  { name: 'Advanced', order: 6, minQuestions: 71, maxFailures: 0, description: 'Nivel avanzado' }
];
...
const examTypes = [
  {
    ...
    passingScore: 70.0,
    timeLimitSeconds: 60,
    description: 'Simulador de práctica'
  },
  {
    name: 'Final',
    totalQuestions: 40,
    ...
    passingScore: 70.0,
    timeLimitSeconds: 60,
    description: 'Simulador final de ubicación'
  }
];

// 80 Preguntas (75 originales + 5 adicionales)
const questions = [
  // BEGINNER (1-10)
  {
    level: 'Beginner',
  ...
  try {
    ...
    for (const level of levels) {
      await pool.request()
        ...
        .input('minQuestions', sql.Int, level.minQuestions)
        .input('maxFailures', sql.Int, level.maxFailures)
        .input('description', sql.VarChar(255), level.description)
        .query(`
          IF NOT EXISTS (SELECT 1 FROM Levels WHERE level_name = @name)
          INSERT INTO Levels (level_name, level_order, min_questions, max_failures, description)
          VALUES (@name, @order, @minQuestions, @maxFailures, @description)
        `);
